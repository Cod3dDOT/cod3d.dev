...
export class Flowmap {
	...
	updateMouse = (e: MouseEvent | TouchEvent) => {
		let xEvent: number | undefined = undefined;
		let yEvent: number | undefined = undefined;

		const touchEvent = e as TouchEvent;
		if (touchEvent.changedTouches?.length) {
			xEvent = touchEvent.changedTouches[0].pageX;
			yEvent = touchEvent.changedTouches[0].pageY;
		}

		const mouseEvent = e as MouseEvent;
		if (xEvent === undefined || yEvent === undefined) {
			xEvent = mouseEvent.pageX;
			yEvent = mouseEvent.pageY;
		}
		...
		const x = xEvent / viewportSize.width;
		const y = 1 - yEvent / viewportSize.height;
